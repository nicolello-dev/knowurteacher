...
export default function SearchByName() {
  ...
  useEffect(() => {
    searched &&
      fetch(
        `/api/moreTeachers?name=${searched}&startIndex=${cursor}&school=${school}`,
      )
        .then((r) => r.json())
        .then((t) => {
          setLoading(false);
  ...
  return (
    <>
      <Head>
        <title>
          {loading
            ? "Teacher search | Knowurteacher"
            : `Search - ${searched} | knowurteacher`}
        </title>
      </Head>
      <Header />
      <section className="p-8 pb-4 flex flex-col items-center dark:text-white">
        <h1 className="text-3xl">Your search results:</h1>
        {loading ? (
        ...
        <div className="flex flex-row flex-wrap justify-evenly">
          {
            ...
            shownTeachers.map((t, key) => (
              <div className="w-72 p-4" key={key}>
                <TeacherProfilePreview teacher={t} button={true} />
              </div>
            ))
          }
        </div>
        {shownTeachers.length < count && (
          <div className="w-full flex flex-row space-around">
            <ul className="flex flex-row mx-auto gap-4 items-baseline">
              <li className="page-item">
                <button
                  className="bg-secondary p-2"
                  onClick={(_) => getPrevResults()}
                >
                  Prev.
                </button>
              </li>
              <li className="page-item">
                <p className="page-link">
                  {shownTeachers.length > 1
                    ? `${cursor + 1}-${cursor + shownTeachers.length}`
                    : cursor + 1}
                </p>
              </li>
              <li className="page-item">
      ...
      <section className="p-8 pb-4 flex flex-col items-center dark:text-white">
        <h1 className="text-2xl">Need more filters?</h1>
        <form
          className="m-8 w-4/6 max-w-96 min-w-72"
          action="/"
          onSubmit={(e) => e.preventDefault()}
        >
          <div className="flex flex-row flex-nowrap mb-3 rounded">
            <p className="m-0 p-2">School:</p>
            <input
              type="text"
              name="school"
              ...
              aria-label="school"
              value={schoolInput}
              onChange={(e: any) => setSchoolInput(e.target.value)}
            />
          </div>
        </form>
        <button
          className="bg-primary p-2 text-lg rounded-lg text-white"
          onClick={(_) => setSchool(schoolInput)}
        >
          Search
        </button>
      </section>
