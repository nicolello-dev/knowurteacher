...
export default async function suggestTeachers(
  req: APIRequest,
  res: NextApiResponse<APIResponse<TeacherSuggestion[]>>,
) {
  if (req.method !== "GET") {
    res.status(405).json({
      success: false,
  ...
  try {
    const teachers: TeacherSuggestion[] =
      await prisma.$queryRaw`SELECT name, school FROM public."Teacher" t WHERE LOWER(t.name) ILIKE ${`%${nameInput}%`} LIMIT 5;`;
