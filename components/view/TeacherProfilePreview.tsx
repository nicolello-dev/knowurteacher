    ...
    >
      <div className="flex flex-col items-center flex-auto p-6">
        <Image
          className="mb-4"
          src={
            teacher.photoURL || "https://cdn.knowurteacher.com/defaultpfp.png"
          }
          height={200}
          width={200}
          alt={`${teacher.name}'s picture`}
        />
        <h1 className="mb-3 text-lg dark:text-darktext">{teacher.name}</h1>
        <p className="mb-0 text-center dark:text-darktext">{teacher.school}</p>
        {button && (
          <a
            href={`/view/${teacher.school}/${teacher.name}`}
